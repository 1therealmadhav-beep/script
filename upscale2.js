(function(){
  ...
  console.log('%c[Image Saver] v2 – Active with blob support', 'color: #06b6d4; font-weight: bold');
  ...
  // --- State ---------------------------------------------------------------
  let allImages = [];            // array of { url, type, mime, fromUpscale }
  ...
  // Check if URL likely points to an image (including blobs with context)
  function isImageUrl(url, mimeType = '', trusted = false) {
    if (!url) return false;
    if (url.startsWith('blob:') || url.startsWith('data:')) {
      // Trust if it came from an intercepted image response
      if (trusted) return true;
      // Otherwise, check MIME if available
  ...
  // --- Add image to collection (with optional MIME) ------------------------
  function captureImageURL(url, fromUpscale = false, mimeType = '') {
    url = resolveURL(url);
    const trusted = fromUpscale; // trust if it came from an interception

    if (!isImageUrl(url, mimeType, trusted)) {
      console.log('[Image Saver] Skipped non-image:', url);
      return;
    }

    // Check if already exists (by URL)
    const exists = allImages.some(item => item.url === url);
    if (!exists) {
      const imageEntry = {
        url: url,
        type: mimeType || 'image/unknown',
        ext: getExt(url) || (mimeType.split('/')[1] || 'img'),
        fromUpscale: fromUpscale
      };
      allImages.push(imageEntry);
      if (fromUpscale) upscaledSet.add(url);
      console.log(`[Image Saver] Captured ${fromUpscale ? '(upscaled)' : ''}:`, url);
    } else if (fromUpscale && !upscaledSet.has(url)) {
      upscaledSet.add(url);
      const entry = allImages.find(item => item.url === url);
      if (entry) entry.fromUpscale = true;
    }
  ...
  function scanPageImages() {
    ...
    document.querySelectorAll('source[srcset]').forEach(s => {
      (s.srcset || '').split(',').forEach(v => {
        ...
        if (url && isImageUrl(url) && !seen.has(resolveURL(url))) {
          captureImageURL(resolveURL(url), false);
          seen.add(resolveURL(url));
        }
      });
    });
  }
  ...
  const observer = new MutationObserver(muts => {
    ...
    muts.forEach(m => m.addedNodes.forEach(node => {
      ...
      if (node.tagName === 'IMG') {
        ...
        if (src && isImageUrl(src) && !allImages.some(item => item.url === resolveURL(src))) {
  ...
  function blockDownload(url, mimeType = '', method = 'unknown') {
    if (!url) return false;
    console.log(`[Image Saver] Blocked ${method}:`, url);
    captureImageURL(url, true, mimeType);
    toast(`🚫 Auto‑download blocked (${method}) – saved to list`, ACCENT_COLOR);
    return true;
  }
  ...
  window.fetch = function(...args) {
    return origFetch.apply(this, args).then(async resp => {
      ...
      if (isAttachment && ctype.startsWith('image/')) {
        ...
        blockDownload(url, ctype, 'fetch attachment');
  ...
  XHR.send = function() {
    this.addEventListener('load', function() {
      ...
      if (disp.includes('attachment') && ctype.startsWith('image/')) {
        ...
        const url = URL.createObjectURL(blob);
        blockDownload(url, ctype, 'XHR attachment');
  ...
  HTMLAnchorElement.prototype.click = function() {
    if (this.href) {
      // If it's a blob URL without extension, still treat as potential image
  ...
  window.open = function(url, ...args) {
    if (url && (url.startsWith('blob:') || url.startsWith('data:') || isImageUrl(url))) {
  ...
  if (originalLocationDescriptor?.configurable) {
    Object.defineProperty(window, 'location', {
      ...
      set: function(val) {
        if (typeof val === 'string' && (val.startsWith('blob:') || val.startsWith('data:') || isImageUrl(val))) {
  ...
  // --- UI Injection (identical to previous, but with blob badge) -----------
