(function(){
  ...
  // ─── State ───────────────────────────────────────────────────────────
  let allImages = [];            // all image URLs on the page
  let upscaledSet = new Set();   // URLs detected after panel open (upscaled results)
  ...
  function toast(msg, color = ACCENT_COLOR){
    ...
    t.style.cssText = `background:#0f0f13;border-left:4px solid ${color};color:#fff;font:600 12px/1.4 'Segoe UI',system-ui,sans-serif;padding:10px 16px;border-radius:8px;box-shadow:0 8px 20px rgba(0,0,0,0.5);max-width:280px;opacity:0;transform:translateX(10px);transition:all 0.2s;`;
    t.textContent = msg;
  ...
  function interceptAutoDownload(){
    ...
    document.addEventListener('click', e => {
      ...
      e.preventDefault();
      e.stopPropagation();
      captureImageURL(href, true);
      toast('⬇ Auto‑download blocked – saved to list', ACCENT_COLOR);
    }, true);

    // 2. Intercept fetch responses that are images with Content-Disposition: attachment
    ...
    window.fetch = function(...args){
      return origFetch.apply(this, args).then(async resp => {
        ...
        if(isAttachment && contentType.startsWith('image/')){
          ...
          toast('📎 Attachment intercepted – image saved', ACCENT_COLOR);
          return new Response(null, { status: 204 }); // block download
        }
        return resp;
      });
    };
    ...
    XHR.send = function(){
      this.addEventListener('load', function(){
        ...
        if(disp.includes('attachment') && ctype.startsWith('image/')){
          ...
          const url = URL.createObjectURL(blob);
          captureImageURL(url, true);
          toast('📎 XHR attachment intercepted', ACCENT_COLOR);
  ...
  // ─── Watch for dynamically added images (upscale results) ───────────
