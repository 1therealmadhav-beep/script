(function(){
  ...
  console.log('%c[Image Saver] v4 – IndexedDB Storage Enabled', 'color: #06b6d4; font-weight: bold');
  ...
  // --- State ---------------------------------------------------------------
  let allImages = [];                 // array of { id, url, blob, mime, ext, timestamp }
  let currentTab = 'all';             // 'all' or 'upscaled' (upscaled = captured in this session)
  const selected = new Set();         // set of image ids
  let db = null;
  ...
  // --- Add captured image (stores blob in IndexedDB) -----------------------
  async function captureImage(blob, urlHint = '', mimeType = '', fromUpscale = true) {
    ...
    const imageEntry = {
      ...
      ext,
      timestamp,
      sessionCapture: fromUpscale // mark as captured in current session
    };

    try {
      await saveImageToDB(imageEntry);
      allImages.push(imageEntry);
      console.log(`[Image Saver] Saved to IndexedDB:`, { id, mime, timestamp });
      toast(`💾 Image saved to browser storage`, ACCENT_COLOR);
    } catch (e) {
      console.error('Failed to save image to IndexedDB:', e);
      toast('❌ Failed to save image', '#ef4444');
      return;
    }
  ...
  async function initializeStorage() {
    try {
      ...
      allImages = stored.map(entry => ({
        ...entry,
        sessionCapture: false // initially not from current session
      }));
      console.log(`[Image Saver] Loaded ${allImages.length} images from IndexedDB`);
      document.getElementById('_img_count').textContent = allImages.length;
    } catch (e) {
      console.error('IndexedDB initialization failed:', e);
      toast('⚠️ Storage unavailable – images will not persist', '#f59e0b');
    }
    softRefresh();
  }

  // --- Scan page for existing images (optional, but we don't store them) ---
  function scanPageImages() {
    // This function is kept minimal; we only capture upscaled results via interception.
    // Optionally, you could scan and store existing images, but that may clutter storage.
  }

  // --- MutationObserver for new images (captures DOM-added images) --------
    ...
    muts.forEach(m => m.addedNodes.forEach(async node => {
      ...
      if (node.tagName === 'IMG') {
        ...
        if (src && (src.startsWith('blob:') || src.startsWith('data:'))) {
          // Try to fetch the blob from the src (may be revoked)
          try {
            const res = await fetch(src);
            const blob = await res.blob();
            if (blob.type.startsWith('image/')) {
              captureImage(blob, src, blob.type, true);
            }
          } catch (e) {
            // Blob likely revoked, can't capture
          }
        }
      }
  ...
  function blockDownload(blob, url, mimeType, method) {
    ...
    captureImage(blob, url, mimeType, true);
    toast(`🚫 Auto‑download blocked – saved to storage`, ACCENT_COLOR);
  }
  ...
  document.addEventListener('click', e => {
    ...
    e.preventDefault();
    e.stopPropagation();
    // We can't get blob here, but we can fetch it
  ...
  // --- UI Styles (with additional storage indicator) -----------------------
  ...
  function renderGrid() {
    ...
    files.forEach(item => {
      ...
      // Use blob to create object URL for display
      if (item.blob) {
        img.src = URL.createObjectURL(item.blob);
        img.onload = () => URL.revokeObjectURL(img.src); // clean up after load
      } else {
        img.src = item.url || '';
      }
  ...
  async function rescan() {
    // Scan page for new images that may have been added
    toast('Scanning page...', ACCENT_COLOR);
    // This is just a trigger; the MutationObserver already captures new images.
  }
  ...
  toast('💾 Image Saver – Persistent storage enabled', ACCENT_COLOR);
