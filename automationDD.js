(function(){
  ...
  const typeOf = u => {
    if (u.startsWith('blob:')) return 'audio';
    const e = ext(u);
  ...
  const sel = new Set();
  let autoDownloadActive = false;      // auto-download on detection (unreliable without gesture)
  let saveOnPlayActive = false;        // NEW: download when user clicks play
  ...
  // Helper: actually save a file (works from a user gesture)
  async function saveFile(url, suggestedName) {
    try {
      // If it's a blob: URL, fetch it first to get real data
      let blob;
      if (url.startsWith('blob:')) {
        const response = await fetch(url);
        blob = await response.blob();
      } else {
        // Normal URL: try to fetch or just use anchor
        const response = await fetch(url);
        blob = await response.blob();
      }
      ...
      a.href = objUrl;
      a.download = suggestedName || fname(url);
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(objUrl);
      toast(`✓ Saved: ${suggestedName || fname(url)}`, '#22c55e');
    } catch (err) {
      // Fallback: try direct anchor (may fail for cross-origin)
      const a = document.createElement('a');
      a.href = url;
      a.download = suggestedName || fname(url);
      a.target = '_blank';
      ...
      a.click();
      document.body.removeChild(a);
      toast(`⚠️ Fallback save attempted: ${fname(url)}`, '#f97316');
    }
  }

  // Auto-download (only works if called from a user gesture)
  function autoDownloadFile(url) {
    if (!autoDownloadActive) return;
    if (autoDownloaded.has(url)) return;
    const fileType = typeOf(url);
    if (fileType !== 'audio') return;
    autoDownloaded.add(url);
    toast(`⚡ Auto-download triggered for ${fname(url)} – may be blocked by browser`, '#f97316');
    // Use saveFile, but it will likely be blocked if no user gesture.
    // We'll still try – if blocked, user will see a toast.
    saveFile(url, fname(url));
  }
  ...
  function catchUrl(raw) {
    ...
    softRefresh();
    // Auto-download (if enabled) – may be blocked
    if (autoDownloadActive && typeOf(url) === 'audio') {
      autoDownloadFile(url);
    }
  }

  // ---------- SAVE ON PLAY (listens to play events) ----------
  function setupSaveOnPlayListener() {
    document.body.addEventListener('play', (e) => {
      const target = e.target;
      if (!target) return;
      if (!saveOnPlayActive) return;
      let src = target.currentSrc || target.src;
      if (!src) return;
      if (typeOf(src) === 'audio') {
        toast(`🎵 Saving audio because you played it: ${fname(src)}`, '#a855f7');
        saveFile(src, fname(src));
      }
    }, true); // capture phase to catch dynamically added elements
  }
  setupSaveOnPlayListener();
  ...
  css.textContent = `
    ...
    .tbbtn.blue{background:rgba(139,92,246,.1);color:#a78bfa;border:1px solid rgba(139,92,246,.22);}
  ...
  ov.innerHTML = `
    <div id="_fd_modal">
      ...
      <div id="_fd_tb">
        ...
        <button class="tbbtn dim" id="_fd_autodl">⚡ Auto-DL OFF</button>
        <button class="tbbtn dim" id="_fd_saveplay">🎵 Save on Play OFF</button>
      </div>
      <div id="_fd_tabs"></div>
      <div id="_fd_mp">
        <div id="_fd_mpi">
  ...
  function doDownload(){ const targets=sel.size?[...sel]:filtered(); if(!targets.length){ toast('No files!','#ef4444'); return; } targets.forEach((url,i)=>setTimeout(()=>{ saveFile(url, fname(url)); },i*300)); toast(`⬇ Downloading ${targets.length} file${targets.length>1?'s':''}…`,'#a855f7'); }
  ...
  // Auto-download toggle (unreliable without gesture)
  const autoBtn = document.getElementById('_fd_autodl');
  autoBtn.onclick = () => {
    autoDownloadActive = !autoDownloadActive;
    autoBtn.textContent = autoDownloadActive ? '⚡ Auto-DL ON' : '⚡ Auto-DL OFF';
    autoBtn.className = 'tbbtn ' + (autoDownloadActive ? 'blue' : 'dim');
    toast(autoDownloadActive ? 'Auto-download ENABLED (may be blocked by browser)' : 'Auto-download DISABLED', autoDownloadActive ? '#f97316' : '#6b7280');
  };
  
  // Save on Play toggle (works because user clicks play)
  const savePlayBtn = document.getElementById('_fd_saveplay');
  savePlayBtn.onclick = () => {
    saveOnPlayActive = !saveOnPlayActive;
    savePlayBtn.textContent = saveOnPlayActive ? '🎵 Save on Play ON' : '🎵 Save on Play OFF';
    savePlayBtn.className = 'tbbtn ' + (saveOnPlayActive ? 'blue' : 'dim');
    toast(saveOnPlayActive ? 'When you play any audio, it will be saved automatically' : 'Save on Play disabled', saveOnPlayActive ? '#22c55e' : '#6b7280');
  };
