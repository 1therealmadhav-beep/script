(function(){
  ...
  const sel = new Set();
  let autoDownloadActive = false;               // ← auto-download toggle
  const autoDownloaded = new Set();             // prevent duplicate auto-downloads
  ...
  function autoDownloadFile(url) {
    ...
    if (fileType !== 'audio') return;   // only audio files auto-download
    autoDownloaded.add(url);
    const a = document.createElement('a');
    a.href = url;
    a.download = fname(url);
    a.target = '_blank';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    toast(`⚡ Auto-downloaded: ${fname(url)}`, '#22c55e');
  }
  ...
  function catchUrl(raw) {
    ...
    softRefresh();
    // Auto-download if enabled and it's an audio file
  ...
  autoBtn.onclick = () => {
    ...
    toast(autoDownloadActive ? 'Auto-download ENABLED for new audio' : 'Auto-download DISABLED', autoDownloadActive ? '#22c55e' : '#6b7280');
  };
