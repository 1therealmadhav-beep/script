(function () {
  // ── Remove any old instance so re-running always works ──
  ['_fd_fab','_fd_overlay','_fd_toast_wrap'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.remove();
  });
  if (window.__IRV_FILE_ARRAY__) {
    delete window.__IRV_FILE_ARRAY__;
    delete window.__IRV_PATCHED__;
  }
  ...
  const isFile   = u => u && ALL_EXT.has(ext(u));
  const typeOf   = u => { const e=ext(u); for(const[t,es]of Object.entries(FILE_TYPES))if(es.includes(e))return t; return 'other'; };
  const isAV     = u => ['audio','video'].includes(typeOf(u));
  const fname    = u => { try{return decodeURIComponent(u.split('?')[0].split('/').pop())||u;}catch{return u;} };
  ...
  // ─────────────────────────────────────────
  //  ① RUNTIME DETECT (XHR + fetch + Audio)
  // ─────────────────────────────────────────
  if (!window.__IRV_PATCHED__) {
    ...
    XMLHttpRequest.prototype.open = function(m,u,...r){
      try{ catchUrl(u); }catch{}
      return _xhrOpen.call(this,m,u,...r);
    };

    const _fetch = window.fetch;
    window.fetch = function(input,...r){
      try{ catchUrl(typeof input==='string'?input:(input&&input.url)||''); }catch{}
      return _fetch.call(this,input,...r);
    };

    const _Audio = window.Audio;
    window.Audio = function(src){
      const a = new _Audio(src);
      if(src) try{ catchUrl(src); }catch{}
      return a;
    };
  }
  ...
  function collect(){
    ...
    document.querySelectorAll('img').forEach(i=>{ const u=i.currentSrc||i.src; if(u&&u!==location.href)s.add(resolve(u)); });
    document.querySelectorAll('video,audio').forEach(m=>{ if(m.src)s.add(resolve(m.src)); m.querySelectorAll('source').forEach(x=>{ if(x.src)s.add(resolve(x.src)); }); });
    document.querySelectorAll('a[href]').forEach(a=>{ if(a.href&&isFile(a.href))s.add(resolve(a.href)); });
    document.querySelectorAll('picture source[srcset]').forEach(x=>{ (x.srcset||'').split(',').map(v=>v.trim().split(' ')[0]).forEach(u=>{ if(u)s.add(resolve(u)); }); });
  ...
  css.textContent = `
    ...
    .fdempty span{font-size:34px;}

    .fdrow{display:flex;align-items:center;gap:9px;padding:9px 12px;border-radius:10px;background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.05);cursor:pointer;transition:background .12s,border-color .12s;position:relative;}
    .fdrow:hover{background:rgba(255,255,255,.05);border-color:rgba(255,255,255,.09);}
    .fdrow.on{background:rgba(255,255,255,.055);border-color:var(--ac);}
    .fdrow.on::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:var(--ac);border-radius:3px 0 0 3px;}

    .fdck{width:16px;height:16px;border-radius:4px;border:2px solid rgba(255,255,255,.13);flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:9px;transition:background .12s,border-color .12s;}
    ...
    .fdplay.going{background:#fff;color:#111;animation:_fd_pu 1s infinite;}
  ...
  //  MINI PLAYER
  // ─────────────────────────────────────────
  const mp   = document.getElementById('_fd_mp');
  ...
  document.getElementById('_fd_mx').onclick = () => {
    maud.pause(); maud.src=''; mp.classList.remove('on');
  ...
  // ─────────────────────────────────────────
  function filtered(){
    let f = currentTab==='all' ? allFiles : allFiles.filter(u=>typeOf(u)===currentTab);
    const q = document.getElementById('_fd_q').value.trim().toLowerCase();
    if(q) f=f.filter(u=>u.toLowerCase().includes(q)||fname(u).toLowerCase().includes(q));
    return f;
  }
  ...
  function updateInfo(){
    const n=sel.size;
    document.getElementById('_fd_si').textContent = n ? `${n} selected` : `${allFiles.length} file${allFiles.length!==1?'s':''} found`;
    document.getElementById('_fd_cnt').textContent = allFiles.length;
  }
  ...
  function renderTabs(){
    ...
    bar.innerHTML='';
    ['all',...Object.keys(FILE_TYPES)].forEach(t=>{
      const files = t==='all' ? allFiles : allFiles.filter(u=>typeOf(u)===t);
      if(t!=='all' && !files.length) return;
      ...
      tab.onclick=()=>{ currentTab=t; renderTabs(); renderGrid(); };
      bar.appendChild(tab);
    });
  }
  ...
  function rescan(){
    ...
    toast(added>0?`🔄 +${added} new file${added>1?'s':''}`:'🔄 No new files found', added>0?'#22c55e':'#6b7280');
    renderTabs(); renderGrid();
  }
  ...
  // ─────────────────────────────────────────
  function doDownload(){
    const targets=sel.size?[...sel]:filtered();
    if(!targets.length){ toast('No files!','#ef4444'); return; }
    targets.forEach((url,i)=>setTimeout(()=>{
      const a=document.createElement('a');
      a.href=url; a.download=fname(url); a.target='_blank';
      document.body.appendChild(a); a.click(); document.body.removeChild(a);
    },i*300));
    toast(`⬇ Downloading ${targets.length} file${targets.length>1?'s':''}…`,'#a855f7');
  }
  ...
  let rtOn=true;
  document.getElementById('_fd_rtbtn').onclick=function(){
    rtOn=!rtOn;
    this.textContent=`🔴 Runtime: ${rtOn?'ON':'OFF'}`;
    this.className='tbbtn '+(rtOn?'orange':'dim');
  };

  document.getElementById('_fd_sall').onclick=()=>{
    const f=filtered();
    f.every(u=>sel.has(u))?f.forEach(u=>sel.delete(u)):f.forEach(u=>sel.add(u));
    renderGrid();
  };

  document.getElementById('_fd_clr').onclick=()=>{ sel.clear(); renderGrid(); };

  // Auto-open so user sees it works immediately
  open();
